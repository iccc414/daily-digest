...
export const SOURCES: Source[] = [
  ...
  // ---- ビジネス/投資 ----
  {
    name: 'Crunchbase Music',
    url: 'https://news.crunchbase.com/tag/music/feed/',
    category: 'ビジネス/投資',
    language: 'en',
    priority: 2,
    trustLevel: 'medium',
...
export const PROMPTS = {
  articleSummary: `あなたは世界の音楽業界に精通した専門アナリストです。以下の英語記事を分析し、日本語のみで回答してください。企業名・サービス名・作品名・アーティスト名などの固有名詞は原語のまま保持してください。

記事リスト:
{ARTICLES}

各記事について以下のJSON形式のみで出力してください（コードフェンスや余分なテキスト不要）:
{"articles":[{"title":"日本語タイトル","summary":"何が起きたか・なぜ重要か・誰に影響するか・今後どう見ればいいかを含む3文以内","importance":"high|medium|low","category":"芸術性|商業性|トレンド|ビジネス|周辺市場","signals":"この情報が業界に与えるインプリケーション1文","url":"元記事URL","sourceName":"ソース名"}]}`,

  executiveSummary: `あなたは世界の音楽業界に精通した専門アナリストです。以下の本日の音楽業界ニュース要約から、最重要トピック3〜7つを抽出し、エグゼクティブサマリーを日本語で作成してください。

ニュース一覧:
{SUMMARIES}

以下のJSON形式のみで出力してください（コードフェンスや余分なテキスト不要）:
{"executive_summary":"本日の音楽業界全体を俯瞰した概況（2〜3文）","top_topics":[{"topic":"トピック名","significance":"なぜ今日重要なのかの説明"}],"signals_and_insights":["複数の情報源から読み取れるトレンドや変化","中長期的に重要な示唆","今後注目すべき動向や企業・人物"]}`,
};
